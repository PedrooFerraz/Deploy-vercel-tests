...
fetch(`${URL}/${auxID}`).then(res => res.json()).then(user => {
  ...
  fetch(`${URL}/${auxID}`).then(res => res.json()).then(users => {
    ...
    var dadosDoGrafico = {
      ...
      datasets: [{
        ...
        data: [diasTreino[10], diasTreino[11], diasTreino[0], diasTreino[1]],
      ...
      options: {
        scales: {
          y: {
            beginAtZero: true,
            suggestedMin: 0,
            // Força o valor mínimo a ser zero
            stepSize: 1
          }
        }
      }
